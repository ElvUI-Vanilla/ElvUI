...
local LibCompress = LibStub:NewLibrary("LibCompress", 90000 + tonumber(string.match("$Revision: 77 $", "%d+")))
...
function LibCompress:CompressLZW(uncompressed)
	if type(uncompressed) == "string" then
		...
		for i = 1, string_len(uncompressed) do
...
function LibCompress:CompressHuffman(uncompressed)
	...
	if string_len(uncompressed) == 0 then
		return "\001"
	end
	...
	if (string_len(uncompressed) + 1) <= string_len(compressed_string) then
...
function LibCompress:DecompressHuffman(compressed)
	...
	local compressed_size = string_len(compressed)
	--decode header
...
function LibCompress:Compress(data)
	...
	while method do
		...
		if string_len(n) < string_len(result) then
			result = n
		end
		method = next(compression_methods, method)
	end
...
local function escape_for_gsub(str)
	return string_gsub(str, "([%z%(%)%.%%%+%-%*%?%[%]%^%$])",  gsub_escape_table)
end
...
function LibCompress:GetEncodeTable(reservedChars, escapeChars, mapChars)
	...
	if getn(mapChars) > 0 then
		...
		table_insert(decode_func_string, "str = string_gsub(str, self.decode_search"..tostring(escapeCharIndex)..", self.decode_translate"..tostring(escapeCharIndex)..");")
	...
	decode_func_string[gen(decode_func_string)] = string_gsub(decode_func_string[getn(decode_func_string)], "str = (.*);", "return %1;")
	...
	encode_func = assert(loadstring("return function(self, str) return string_gsub(str, self.encode_search, self.encode_translate); end"))()
